...
#define KEY_ESC 27 // A
static vector<unsigned char> imgData;
//
...
/// Later on
static bool flag = 1;
static bool TroopShow = 0;
static int playernum = 2;
static float* playercolor[6]{};
static string gameState = "menu";   // Initial State
static int currentPlayer = 1;
static int shadow = 6;
static const int width = 1024, height = 800, channels = 3;
static int background[width / 16][height / 16];
static int colorMod[width / 16][height / 16];
static bool bgDrawn;
static bool inc = 1;
...
static bool attack3;
static bool card;
// To justify war result
static int just;

// Creating an Array to store the color of each troop in a territory
static float* terrcolor[51][3] = { nullptr };
// Making Cards for 6 players , 3 cols , one for each type of card and number of cards count
static int cardss[6][3] = {0};

// Making Scores array to store the 10 highest scores from the file 
static int scores[10]{};
// factor for size of board
static int factor = 0;
// Making Names array to store the names of 10 players with highest scores
static string names[10]{}; //= {"test" , "test", "test" , "test" , "test" , "test" , "test" , "test" , "test" , "test" };
// Bool to identify whether the player can fortify
static bool fortification = 0;
// To check if a certain territory is selected for fortification
static bool selectt = 1;
// Making names and scores array for current players 
static int cscore[6] = {0};
static string cnames[6]{}; //= {"Player_1" , "Player_2" , "Player_3", "Player_4" , "Player_5" , "Player_6" };
// To know the player with highest dice rolled 
static int maximum = -1;
// To store a number 1-6 for each player
static int territory[51]{};
// To Store info of troops in a certain territory
static int terr[51][3];
// Number of territories of a certain player
static int terrNum[6]{};
// Number of certain troops
static int troopNum[6][3]{};
// Count to check if the all the territories have been occupied once
static int terrcount = 0;
// Chosen Territory by the player on map
static int chosen = 0;
// Highest Score and Player in the current game 
static int newscore = 1;
// Dice for each player
static int dice[6]{};
// To Show the Troops in a certain territory when hovered over
static bool show[51]{};
// To store the name of player with the highest score
static string newname; //= cnames[]
